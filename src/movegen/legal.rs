...
mod tests {
    ...
    fn test_en_passant_legality() {
        ...
        // Wait — after en passant, King on a5 would be exposed to Rook d5
        // That en passant should be illegal
        let pos = Position::from_fen(fen).unwrap();
        let legal = generate_moves(&pos);
        let ep_moves: Vec<_> = legal.iter()
            .filter(|m| m.kind == MoveKind::EnPassant)
            .collect();
        // En passant should be filtered out as it exposes King
        assert_eq!(ep_moves.len(), 0,
            "En passant that exposes King should be illegal");
    }
